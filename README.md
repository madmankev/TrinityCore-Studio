...
5. **Sculpt terrain safely:** use **Terrain Sculpt** to arm Raise, Lower, Flatten, **Smooth**,
   ...
   noise; stamps provide Hill, Valley, Crater, and yaw-rotated Ridge presets. **Terrain Paint** now
   has two authoring lanes: sampled **MCLY/MCAL texture-layer paint** for existing base/overlay slots,
   plus MCCV per-vertex tint. Sample a clicked MCNK to see its real MTEX names, choose a slot, then
   paint/erase/reveal with radius and opacity. Height tools preview immediately; texture and MCCV
   strokes are staged, undoable, paint only existing MCAL-backed assignments, repair offsets on save, and appear
   after **Save ADT edits** reloads the project overlay. All tools preserve/rebuild the appropriate
   ADT payloads and support Ctrl+Z/Ctrl+Y.
