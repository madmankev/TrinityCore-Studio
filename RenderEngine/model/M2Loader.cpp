...
namespace { void ResolveGeoParticles(ClientData& cd, M2Model& out);
            void ResolveRecursionParticles(ClientData& cd, M2Model& out); }
          ...
          bool requireGeometry)
{
    ...
    // Vertices. Recursion child models are particle-only (no mesh); allow empty there.
    if (!r.GetArray(h.vertices, out.vertices) || (out.vertices.empty() && requireGeometry))
    {
    ...
    out.materials = materials;   // kept for ribbon materialIndices -> blendMode resolution
    ...
    // Attachment points (weapon/helm/shoulder mounts).
    std::vector<M2AttachmentDef> attDefs;
    r.GetArray(h.attachments, attDefs);
    out.attachments.reserve(attDefs.size());
    for (const M2AttachmentDef& a : attDefs)
        out.attachments.push_back({a.id, a.bone, {a.position[0], a.position[1], a.position[2]}});
    ...
    // Particle-only model (a recursion child): no geometry/.skin — its emitters, bones and
    // tracks are already parsed. Return before the geometry pipeline.
    if (out.vertices.empty())
    {
        out.boundsCenter = glm::vec3(0.0f);
        out.boundsRadius = 1.0f;
        out.name = m2Path;
        return true;
    }
    ...
    for (const M2Batch& b : batches)
    {
        ...
        rb.shaderId = b.shaderId;
        rb.materialLayer = b.materialLayer;
        rb.textureCount = b.textureCount;
    ...
    if (g_loadDepth == 1)
    {
        ...
        ResolveRecursionParticles(cd, out);
    }
...
void ResolveRecursionParticles(ClientData& cd, M2Model& out)
{
    out.particleRecursionModel.assign(out.particleEmitters.size(), -1);
    if (out.particleEmitters.empty())
        return;

    Reader r{out.m2Bytes.data(), out.m2Bytes.size()};
    std::unordered_map<std::string, int> byPath;
    // Child textures append AFTER the main slots and all geometry-particle textures.
    int textureBase = static_cast<int>(out.texturePaths.size());
    for (const GeoParticleModel& g : out.geoParticleModels)
        textureBase += static_cast<int>(g.textures.size());

    for (size_t ei = 0; ei < out.particleEmitters.size(); ++ei)
    {
        std::string fn = r.GetString(out.particleEmitters[ei].recursionModelFilename);
        if (fn.empty())
            continue;
        fn = NormalizeModelPath(fn);
        std::string key = ToLower(fn);
        ...
        if (auto it = byPath.find(key); it != byPath.end())
        {
            out.particleRecursionModel[ei] = it->second;
            continue;
        }

        auto child = std::make_unique<M2Model>();
        // Loaded at depth 2 (child's own geo/recursion not resolved), geometry-optional since
        // recursion children are usually particle-only (vertex-less) models.
        if (!Load(cd, fn, *child, nullptr, /*requireGeometry=*/false) || child->particleEmitters.empty())
        {
            byPath[key] = -1;
            continue;
        }
        ...
        RecursionModel rm;
        rm.textureBase = textureBase;
        rm.textures.resize(child->texturePaths.size());
        for (size_t i = 0; i < child->texturePaths.size(); ++i)
        {
            if (child->texturePaths[i].empty())
                continue;
            BlpImage img = DecodeBlp(cd.ReadFile(child->texturePaths[i]));
            if (img.valid())
            {
                rm.textures[i].rgba = std::move(img.rgba);
                rm.textures[i].w = img.width;
                rm.textures[i].h = img.height;
            }
        }
        textureBase += static_cast<int>(rm.textures.size());
        rm.model = std::move(child);

        int idx = static_cast<int>(out.recursionModels.size());
        out.recursionModels.push_back(std::move(rm));
        byPath[key] = idx;
        out.particleRecursionModel[ei] = idx;
    }
}
} // namespace
} // namespace we::m2
