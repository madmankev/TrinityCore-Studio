...
ModelUpload BuildUpload(ClientData& cd, const M2Model& model)
{
    ...
    // Then recursion (child) particle textures, in the same order the loader assigned their
    // textureBase (right after the geometry-particle textures).
    for (const RecursionModel& rm : model.recursionModels)
    {
        for (const GeoParticleModel::Tex& t : rm.textures)
        {
            ModelTextureGpu mt;
            mt.rgba = t.rgba;
            mt.w = t.w;
