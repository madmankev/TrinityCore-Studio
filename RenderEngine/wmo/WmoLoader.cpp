                 ...
                 std::unordered_map<std::string, std::pair<int, int>>& frameCache, WmoModel& out)
{
    ...
    for (uint32_t row = 0; row < yt; ++row)
        for (uint32_t col = 0; col < xt; ++col)
        {
            ...
            const uint8_t fl = fi < tileFlags.size() ? tileFlags[fi] : 0x0F;
            if ((fl & 0x0F) == 0x0F)   // low-nibble 0x0F sentinel: tile has no liquid / hidden
                continue;
...
// Apply the client's CMapObjGroup::FixColorVertexAlpha to the color MOCV (the chunk
// referenced by group flag 0x4) before we bake it as vertex color. Without it, interior
// lighting is subtly wrong — fireplaces/braziers don't glow and bellows light is
// undersaturated. Mirrors the decompiled WotLK routine (wowdev MOCV section): the vertices
// up to the trans-batch split get one transform, the rest another; the group's base
// (ambient) color is subtracted unless the unified render path (MOHD 0x02) adds it at
// runtime instead. MOHD 0x08 requests the simplified alpha-only path. `mocv`/`moba` are the
// group's chunks; `ambColor` is the MOHD ambient color (BGRA). Operates in place on the
// packed BGRA values so the existing bake loop is unchanged.
void FixColorVertexAlpha(std::vector<uint32_t>& mocv, const std::vector<WmoBatch>& moba,
                         uint16_t transBatchCount, uint32_t mohdFlags, uint32_t ambColor,
                         uint32_t groupFlags)
{
    const uint8_t exteriorAlpha = (groupFlags & 0x8) ? 0xFF : 0x00;   // SMOGroup::EXTERIOR

    // First vertex covered by the "second" fixup: one past the last vertex of the last
    // transition batch (batchCounts[0] == transBatchCount). Zero when there are none.
    size_t beginSecond = 0;
    if (transBatchCount > 0 && transBatchCount <= moba.size())
        beginSecond = static_cast<size_t>(moba[transBatchCount - 1].maxIndex) + 1;
    beginSecond = std::min(beginSecond, mocv.size());

    auto unpack = [](uint32_t p, int& r, int& g, int& b, int& a) {
        b = p & 0xFF; g = (p >> 8) & 0xFF; r = (p >> 16) & 0xFF; a = (p >> 24) & 0xFF;
    };
    auto pack = [](int r, int g, int b, int a) -> uint32_t {
        return uint32_t(b & 0xFF) | (uint32_t(g & 0xFF) << 8) | (uint32_t(r & 0xFF) << 16) |
               (uint32_t(a & 0xFF) << 24);
    };
    auto clamp255 = [](int v) { return v < 0 ? 0 : (v > 255 ? 255 : v); };

    // MOHD 0x08 (do_not_fix_vertex_color_alpha): only rewrite the second region's alpha.
    if (mohdFlags & 0x8)
    {
        for (size_t i = beginSecond; i < mocv.size(); ++i)
        {
            int r, g, b, a; unpack(mocv[i], r, g, b, a);
            mocv[i] = pack(r, g, b, exteriorAlpha);
        }
        return;
    }

    int baseR = 0, baseG = 0, baseB = 0;
    if (!(mohdFlags & 0x2))   // unified render path adds MOHD color at runtime — don't bake it
    {
        baseB = ambColor & 0xFF; baseG = (ambColor >> 8) & 0xFF; baseR = (ambColor >> 16) & 0xFF;
    }

    for (size_t i = 0; i < beginSecond; ++i)   // transition-batch vertices
    {
        int r, g, b, a; unpack(mocv[i], r, g, b, a);
        const float af = a / 255.0f;
        r = clamp255(static_cast<int>(((r - baseR) - af * (r - baseR)) * 0.5f));
        g = clamp255(static_cast<int>(((g - baseG) - af * (g - baseG)) * 0.5f));
        b = clamp255(static_cast<int>(((b - baseB) - af * (b - baseB)) * 0.5f));
        mocv[i] = pack(r, g, b, a);
    }
    for (size_t i = beginSecond; i < mocv.size(); ++i)   // interior/exterior-batch vertices
    {
        int r, g, b, a; unpack(mocv[i], r, g, b, a);
        r = clamp255(((r * a) / 64 + r - baseR) / 2);
        g = clamp255(((g * a) / 64 + g - baseG) / 2);
        b = clamp255(((b * a) / 64 + b - baseB) / 2);
        mocv[i] = pack(r, g, b, exteriorAlpha);
    }
}
...
void ParseGroup(const std::vector<uint8_t>& bytes, const std::vector<WmoMaterial>& materials,
                bool includeLiquid, uint32_t mohdFlags, uint32_t mohdAmbColor,
                const LiquidTypeTable* liquidTypes, ClientData& cd,
                std::unordered_map<std::string, std::pair<int, int>>& frameCache, WmoModel& out)
{
    ByteReader gr(bytes);
    ...
    while (git.Next(c))
    {
        ...
        // A TVERTS2 group (flag 0x2000000) has two MOTV chunks; base texturing uses the
        // FIRST. Likewise a CVERTS2 group (0x1000000) has a second MOCV whose RGB is not
        // color (only its alpha blends textures) — keep the first MOCV (the 0x4 color one).
        else if (c.Is("MOTV")) { if (motv.empty()) sub.GetArrayAt<float>(c.offset, c.size / 4, motv); }
        ...
        else if (c.Is("MOCV")) { if (mocv.empty()) sub.GetArrayAt<uint32_t>(c.offset, c.size / 4, mocv); }
    ...
    // Only the FIRST MOCV (group flag 0x4) is real color; a group carrying only the CVERTS2
    // blend-alpha chunk (0x4 clear) must NOT bake it as color.
    const bool exterior = (gh.flags & 0x8) != 0;
    const bool hasColorMocv = (gh.flags & kWmoGroupHasVertexColor) != 0;
    const bool useMocv = hasColorMocv && mocv.size() >= nVerts && !exterior;
    if (useMocv)
        FixColorVertexAlpha(mocv, moba, gh.transBatchCount, mohdFlags, mohdAmbColor, gh.flags);
          ...
          const LiquidTypeTable* liquidTypes, std::string* error)
{
    ...
    for (uint32_t g = 0; g < hdr.nGroups; ++g)
    {
        ...
        ParseGroup(gb, materials, opt.liquid, hdr.flags, hdr.ambColor, liquidTypes, cd, frameCache, out);
        ++out.groupCount;
    }
