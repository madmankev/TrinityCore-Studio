...
TextureId ClientAssets::Texture(const std::string& blpPath)
{
    if (!cd || blpPath.empty())
        return 0;
...
TextureId ClientAssets::MapTile(const std::string& dir, int index)
{
...
TextureId ClientAssets::MapOverlayTile(const std::string& dir, const std::string& texture,
...
TextureId ClientAssets::ItemIcon(uint32_t itemEntry)
{
...
TextureId ClientAssets::SpellIcon(uint32_t spellId)
{
    if (!cd || spellId == 0)
        return 0;
