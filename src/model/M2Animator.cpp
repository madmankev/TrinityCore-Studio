...
glm::vec3 M2Animator::SampleVec3(const M2Track& track, int animIndex, float timeMs, glm::vec3 def)
{
    ...
    if (!src.ok)
        return def;
    TrackData td = FetchTrack(model_, track, src.buf.data, src.buf.size, src.subIndex, sizeof(float) * 3);
    if (td.count == 0)
        return def;
    ...
    auto val = [&](uint32_t i) {
        float v[3];
        std::memcpy(v, td.values + i * sizeof(float) * 3, sizeof(v));
...
glm::quat M2Animator::SampleQuat(const M2Track& track, int animIndex, float timeMs, glm::quat def)
{
    ...
    if (!src.ok)
        return def;
    TrackData td = FetchTrack(model_, track, src.buf.data, src.buf.size, src.subIndex, sizeof(M2CompQuat));
    if (td.count == 0)
        return def;
    uint32_t i0, i1; float f;
    FindKeys(td.timestamps, td.count, src.t, i0, i1, f);
    auto val = [&](uint32_t i) {
        M2CompQuat q;
        std::memcpy(&q, td.values + i * sizeof(M2CompQuat), sizeof(q));
        return UnpackQuat(q);
    };
...
float M2Animator::SampleFloat(const M2Track& track, int animIndex, float timeMs, float def)
{
    ...
    if (!src.ok)
        return def;
    TrackData td = FetchTrack(model_, track, src.buf.data, src.buf.size, src.subIndex, sizeof(float));
    if (td.count == 0)
        return def;
    uint32_t i0, i1; float f;
    FindKeys(td.timestamps, td.count, src.t, i0, i1, f);
    auto val = [&](uint32_t i) {
        float v;
        std::memcpy(&v, td.values + i * sizeof(float), sizeof(v));
        return v;
    };
    return glm::mix(val(i0), val(i1), f);
}
