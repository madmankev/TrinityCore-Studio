...
{
    if (!hasItem || undo_.empty())
        return;
    redo_.push_back(currentItem);
    currentItem = undo_.back();
    undo_.pop_back();
    dirty = true;
    validationDirty = true;
    SetStatus("Undo");
...
{
    if (!hasItem || redo_.empty())
        return;
    undo_.push_back(currentItem);
    currentItem = redo_.back();
    redo_.pop_back();
    dirty = true;
    validationDirty = true;
    SetStatus("Redo");
