...
void GameObjectModule::DrawEditMenu()
{
...
{
    if (!hasGo || undo_.empty())
        return;
    redo_.push_back(currentGo);
    currentGo = undo_.back();
    undo_.pop_back();
    dirty = true;
    validationDirty = true;
    SetStatus("Undo");
...
{
    if (!hasGo || redo_.empty())
        return;
    undo_.push_back(currentGo);
    currentGo = redo_.back();
    redo_.pop_back();
    dirty = true;
    validationDirty = true;
    SetStatus("Redo");
...
void GameObjectModule::PreviewSql()
{
    ...
    exp.SetOutputPath("");
    DbError e = repo.SaveGameObject(exp, currentGo);
