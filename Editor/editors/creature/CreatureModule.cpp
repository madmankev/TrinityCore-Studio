...
void CreatureModule::SetEditedCreature(Creature&& c, bool markDirty)
{
    ...
    hasCreature = true;
    dirty = markDirty;
    undo_.Reset(currentCreature);
    validationDirty = true;
}

void CreatureModule::Undo()
{
    if (!hasCreature || !undo_.CanUndo())
        return;
    currentCreature = undo_.Undo(currentCreature);
    dirty = true;
    validationDirty = true;
    SetStatus("Undo");
}
...
{
    if (!hasCreature || !undo_.CanRedo())
        return;
    currentCreature = undo_.Redo(currentCreature);
    dirty = true;
    validationDirty = true;
    SetStatus("Redo");
}
