...
// Model-local AABB over an upload's vertices (for tight ray-vs-OBB picking).
void AabbOf(const ModelUpload& up, glm::vec3& outMin, glm::vec3& outMax)
{
...
void GameObjectLayer::AddGameObject(const MapGameObject& g)
{
    Go go;
    go.data = g;
    gos_.push_back(std::move(go));
...
bool GameObjectLayer::RemoveGameObject(uint32_t guid)
{
    for (size_t i = 0; i < gos_.size(); ++i)
        if (gos_[i].data.guid == guid)
        {
            gos_.erase(gos_.begin() + i);
            return true;
        }
    return false;
}
...
GameObjectLayer::GoModel* GameObjectLayer::EnsureModel(uint32_t displayId)
{
    ...
    if (gm.isWmo)
    {
        ...
        gm.boundsCenter = wm.boundsCenter;
        gm.boundsRadius = wm.boundsRadius;
        ModelUpload up = wmo::BuildUpload(*cd_, wm);
        AabbOf(up, gm.boundsMin, gm.boundsMax);
    ...
    else
    {
        ...
        if (dresser_)
            dresser_->ResolveDefaultTextures(path, *gm.m2);   // fill runtime skin slots (else white)
        gm.boundsCenter = gm.m2->boundsCenter;
        gm.boundsRadius = gm.m2->boundsRadius;
                            ...
                            std::vector<SceneInstanceGpu>& outScene, int maxDraw, float cullDist)
{
    ...
    for (int k = 0; k < limit; ++k)
    {
        ...
        if (go.data.guid == highlightGuid_ && highlightColor_.a > 0.0f)
        {
        ...
        if (go.data.guid == outlineGuid_ && outlineColor_.a > 0.0f)
        {
...
uint32_t GameObjectLayer::Pick(const glm::vec3& ro, const glm::vec3& rd, const glm::vec3& focus,
                               ...
                               float& outDist)
{
    ...
    for (Go& go : gos_)
    {
        const MapGameObject& g = go.data;
        if (!filter.Visible(g.phaseMask, g.spawnMask, g.eventEntry, g.poolHidden, g.groupManual))
            continue;
        const glm::vec3 dd(g.x - worldFocus.x, g.y - worldFocus.y, g.z - worldFocus.z);
        if (dd.x * dd.x + dd.y * dd.y + dd.z * dd.z > cull2)
            continue;
        const glm::mat4 M = PlacementMatrix(g, origin, nullptr);
        auto it = models_.find(g.displayId);
        // Tight ray-vs-OBB against the model's local AABB; small default box if not yet loaded.
        ...
        const float t = RayObb(ro, rd, M, bmin, bmax);
        if (t >= 0.0f && t < bestT) { bestT = t; best = g.guid; }
    }
    outDist = best ? bestT : -1.0f;
    return best;
}

MapGameObject* GameObjectLayer::FindSpawn(uint32_t guid)
{
    for (Go& go : gos_)
        if (go.data.guid == guid)
            return &go.data;
    return nullptr;
}
