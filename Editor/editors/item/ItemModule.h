...
public:
    ...
    bool CanUndo() const override { return hasItem && undo_.CanUndo(); }
    bool CanRedo() const override { return hasItem && undo_.CanRedo(); }
    void Undo() override;
    void Redo() override;
...
private:
    ...
    // a change when the value actually changed). This editor's OWN stack — the shell
    // routes Ctrl+Z/Y and the Edit-menu items here only while Item is active.
    SnapshotStack<Item> undo_;
