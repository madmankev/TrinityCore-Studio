...
public:
    ...
    // --- undo/redo (per-module; the shell owns only the Edit-menu items + Ctrl+Z/Y
    // shortcuts and dispatches them to the active module). Each module keeps its OWN
    // stack, so undo in one editor never touches another's history. Default no-op so a
    // module can opt out. See editors/common/CommandStack.h + SnapshotStack.h. ---
