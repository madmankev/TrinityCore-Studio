    ...
    bool fill, ImU32 color, float strokeThickness, const ImVec2* startDirHint = nullptr, const ImVec2* endDirHint = nullptr)
{
    ...
    else
    {
        ...
        drawList->PathStroke(color, strokeThickness, ImDrawFlags_Closed);  // [vendor patch] 1.92.8 swapped thickness/flags
    }
}
    ...
    if (flags & Hovered)
    {
        ...
        if (m_BorderWidth > 0.0f)
        {
            ...
            drawList->AddRect(m_Bounds.Min, m_Bounds.Max,
                m_BorderColor, m_Rounding, m_BorderWidth, m_Corners);  // [vendor patch] 1.92.8 swapped thickness/flags
        }
        ...
        if (IsGroup(this))
        {
            ...
            if (m_GroupBorderWidth > 0.0f)
            {
                ...
                drawList->AddRect(
                    m_GroupBounds.Min,
                    m_GroupBounds.Max,
                    m_GroupBorderColor, m_GroupRounding, m_GroupBorderWidth, c_AllRoundCornersFlags);  // [vendor patch] 1.92.8 swapped thickness/flags
            }
        }
...
void ed::Node::DrawBorder(ImDrawList* drawList, ImU32 color, float thickness, float offset)
{
    if (thickness > 0.0f)
    {
        ...
        drawList->AddRect(m_Bounds.Min - extraOffset, m_Bounds.Max + extraOffset,
            color, ImMax(0.0f, m_Rounding + offset), thickness, c_AllRoundCornersFlags);  // [vendor patch] 1.92.8 swapped thickness/flags
    }
}
